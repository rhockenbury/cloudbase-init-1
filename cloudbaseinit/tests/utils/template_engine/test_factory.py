...
class TestTemplateFactory(unittest.TestCase):
    ...
    def test_get_template_manager_empty(self):
        fake_userdata = b''
        result = factory.get_template_manager(fake_userdata)
        self.assertEqual(result, None)

    def test_get_template_manager_no_match(self):
        fake_userdata = b'no match'
        result = factory.get_template_manager(fake_userdata)
        self.assertEqual(result, None)

    def test_get_template_manager_not_supported(self):
        fake_userdata = b'## template:fake'
        self.assertRaises(NotImplementedError,
                          factory.get_template_manager, fake_userdata)

    @mock.patch('cloudbaseinit.utils.classloader.ClassLoader')
    def test_get_template_manager(self, mock_class_loader):
    ...
    @mock.patch('cloudbaseinit.utils.classloader.ClassLoader')
    def test_get_template_manager_class_not_found(self, mock_class_loader):
        fake_userdata = b'## template:jinja'
