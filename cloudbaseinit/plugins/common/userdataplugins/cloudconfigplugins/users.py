...
class UsersPlugin(base.BaseCloudConfigPlugin):
    ...
    def _get_expire_interval(self, data):
        ...
        if isinstance(expiredate, six.string_types):
            ...
            expiredate = datetime(year=year, month=month, day=day,
                                  tzinfo=pytz.utc)
    ...
    def _create_user(self, item, osutils):
            ...
            create_home = not item.get('no_create_home ', False)
            create_user_logon = (not user_disabled and
                                 (public_keys or create_home))
